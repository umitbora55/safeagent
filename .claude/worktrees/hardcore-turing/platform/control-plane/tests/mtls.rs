...
async fn start_server_with_timeout(approval_timeout_secs: u64) -> SocketAddr {
    ...
    let ca = pki_path("ca.crt");
    let _cert = pki_path("control-plane.crt");
    let _key = pki_path("control-plane.key");
    let token_issuer =
        Ed25519TokenIssuer::from_file(&pki_path("token_issuer.key").to_string_lossy())
            .expect("issuer key");
    let worker_client = build_client_config(
        &ca.to_string_lossy(),
        &pki_path("worker.crt").to_string_lossy(),
        &pki_path("worker.key").to_string_lossy(),
    )
...
async fn start_worker_server() -> SocketAddr {
    start_worker_server_with_approval_timeout("https://127.0.0.1:8443", 30).await
}
...
async fn start_worker_server_with_approval_timeout(
    ...
    let public_key = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("..")
        .join("pki")
        .join("token_issuer.pub");
    let verifier =
        Ed25519TokenVerifier::from_file(&public_key.to_string_lossy()).expect("worker verifier");
    let worker_client_config = build_client_config(
        &pki_path("ca.crt").to_string_lossy(),
        &pki_path("worker.crt").to_string_lossy(),
        &pki_path("worker.key").to_string_lossy(),
    )
    .expect("worker client config");
...
async fn execute_via_control_plane_passes() {
    ...
    let worker_addr = start_worker_server().await;
