...
pub fn run_stats(data_dir: &Path) -> Result<()> {
    ...
    let today = ledger.today_summary().map_err(|e| anyhow::anyhow!("{}", e))?;
    let week = ledger.week_summary().map_err(|e| anyhow::anyhow!("{}", e))?;
    let month = ledger.month_summary().map_err(|e| anyhow::anyhow!("{}", e))?;
    let total = ledger.total_summary().map_err(|e| anyhow::anyhow!("{}", e))?;
    ...
    if total.total_input_tokens > 0 {
        let cache_rate = if total.total_cache_read_tokens > 0 {
            ...
            (total.total_input_tokens + total.total_cache_read_tokens) as f64 * 100.0
        } else {
            0.0
        };
        println!("  Cache read tokens: {}  ({:.1}% of total input)",
            total.total_cache_read_tokens, cache_rate);
        println!("  Cache write tokens: {}", total.total_cache_write_tokens);
        println!();
    }

    let breakdown = ledger.model_breakdown_since("1970-01-01T00:00:00")
        .map_err(|e| anyhow::anyhow!("{}", e))?;
    ...
    if !breakdown.is_empty() {
        ...
        println!("  ─────────────────────────   ─────   ──────────");
        for m in &breakdown {
            println!("  {:<27} {:>5}   ${:.4}", m.model_name, m.request_count, m.cost_usd());
        }
        println!();
    }

    let daily = ledger.daily_costs(7).map_err(|e| anyhow::anyhow!("{}", e))?;
    if !daily.is_empty() {
        println!("  Last 7 Days");
        println!("  ──────────────────────────────────────────");
        for d in &daily {
            ...
            println!("  {}  {:>4} reqs  ${:<8.4}  {}", d.date, d.request_count, d.cost_usd(), bar);
        }
        println!();
    }
...
fn print_row(label: &str, s: &safeagent_cost_ledger::CostSummary) {
    println!("  {:<12}    {:>6}      ${:<8.4}   {}/{}",
        label, s.total_requests, s.cost_usd(),
